/**
 * Course and tee catalog (spec §4.3).
 *
 * Hole data arrives either typed by hand or pasted from a spreadsheet, so both
 * forms read their holes through the shared CSV reviewer: a headed export and a
 * bare `hole,par,yards,index` paste parse identically, and the problems come
 * back by row and column instead of as a silent NaN.
 *
 * The completeness check is not housekeeping. `allocateStrokes` throws unless
 * the stroke indexes are a permutation of 1..N, because that permutation is
 * what turns a Playing Handicap into strokes on specific holes — a tee saved
 * with a duplicate index produces an event that cannot be scored at all.
 */
...
import { reviewCourseHolesCsv, type CsvIssue } from '@gtt/contracts';
...
/**
 * Read the hole table, or throw with everything wrong with it. The server
 * validates the same rules again; this exists so the organizer sees which row
 * is wrong rather than a single rejection after the round trip.
 */
function readHoles(value: FormDataEntryValue | null) {
  const report = reviewCourseHolesCsv(String(value ?? ''));
  if (!report.ok) throw new Error(describeIssues(report.issues));
  return report.holes;
}

function describeIssues(issues: readonly CsvIssue[]): string {
  const blocking = issues.filter((issue) => !issue.warning);
  return blocking
    .map((issue) => `${issue.row === 0 ? 'Holes' : `Row ${issue.row}`}${issue.column ? ` (${issue.column})` : ''}: ${issue.message}`)
    .join(' ');
}
...
export function LeagueCourses() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await saveCatalogItem({ action: 'create-course', leagueId, name: form.get('name'), location: form.get('location'), timezone: form.get('timezone'), layoutName: form.get('layoutName'), teeName: form.get('teeName'), ratingCategory: 'standard', courseRating: Number(form.get('courseRating')), slopeRating: Number(form.get('slopeRating')), holes: readHoles(form.get('holes')) });
  ...
  async function addTee(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await saveCatalogItem({ action: 'add-tee', leagueId, courseLayoutId: form.get('courseLayoutId'), teeName: form.get('teeName'), ratingCategory: 'standard', courseRating: Number(form.get('courseRating')), slopeRating: Number(form.get('slopeRating')), holes: readHoles(form.get('holes')) });
