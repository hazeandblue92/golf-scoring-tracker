...
export function ScoreEntry() {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const [{ data: holes }, { data: entries }, { data: scores }, { data: permissions }, { data: roles }] = await Promise.all([
  ...
  const serverScores = useMemo(() => new Map((query.data?.scores ?? []).map((score) => [`${score.event_entry_id}:${score.event_hole_id}`, score])), [query.data?.scores]);
  ...
  useEffect(() => {
    ...
    for (const entry of query.data.entries) {
      ...
      const server = serverScores.get(`${entry.id}:${currentHole.id}`);
      next[entry.id] = draft
  ...
  const { event, round, holes, entries } = query.data;
  window.localStorage.setItem('gtt.activeEventId', event.id);

  function change(entryId: string, next: EditorValue) {
    setValues((current) => ({ ...current, [entryId]: next }));
    setDirty((current) => new Set(current).add(entryId));
  ...
  async function saveHole() {
    ...
    try {
      for (const entryId of dirty) {
        const value = values[entryId];
        if (value === undefined) continue;
        const server = serverScores.get(`${entryId}:${hole.id}`);
        await enqueueScoreMutation({
          eventId,
          roundId: round.id,
          target: { kind: 'individual', entryId, holeId: hole.id },
  ...
  const completed = query.data.scores.filter((score) => score.score_status !== 'not_started').length;
  const totalRequired = holes.length * entries.length;
  ...
  return (
    <div className="score-screen">
      ...
      <section className="score-list" aria-label={`Scores for hole ${currentHole.hole_ordinal}`}>
        {entries.length === 0 ? (
          <div className="empty-state"><h2>No scoring assignment</h2><p>An organizer must assign you this group before you can enter scores.</p></div>
        ) : entries.map((entry) => {
          const value = values[entry.id] ?? { status: 'complete' as const, gross: currentHole.par };
          const server = serverScores.get(`${entry.id}:${currentHole.id}`);
          return (
            <article className="score-row" key={entry.id}>
              <div className="score-person">
                <div className="initials" aria-hidden="true">{initials(entry.participants?.display_name ?? 'Player')}</div>
                <div><h2>{entry.participants?.display_name ?? 'Player'}</h2><span>Playing handicap {entry.playing_handicap ?? '—'} · {server ? `rev ${server.revision}` : 'not entered'}</span></div>
              </div>
              <div className="score-controls">
                <button type="button" aria-label={`Decrease ${entry.participants?.display_name ?? 'player'} score`} onClick={() => change(entry.id, { status: 'complete', gross: Math.max(1, (value.gross ?? currentHole.par) - 1) })} disabled={value.status !== 'complete'}>−</button>
                <input aria-label={`${entry.participants?.display_name ?? 'Player'} gross score`} inputMode="numeric" type="number" min="1" max="25" value={value.gross ?? ''} disabled={value.status !== 'complete'} onChange={(event) => change(entry.id, { status: 'complete', gross: event.target.value === '' ? null : Math.min(25, Math.max(1, Number(event.target.value))) })} />
                <button type="button" aria-label={`Increase ${entry.participants?.display_name ?? 'player'} score`} onClick={() => change(entry.id, { status: 'complete', gross: Math.min(25, (value.gross ?? currentHole.par) + 1) })} disabled={value.status !== 'complete'}>+</button>
              </div>
              <label className="compact-select">Result
                <select value={value.status} onChange={(event) => {
                  ...
                  change(entry.id, { status, gross: status === 'complete' ? value.gross ?? currentHole.par : null });
                }}>
      ...
      <footer className="score-footer">
        ...
        <button className="button button--primary" type="button" onClick={() => void saveHole()} disabled={dirty.size === 0 || saving || entries.length === 0}>{saving ? 'Saving…' : `Save hole ${currentHole.hole_ordinal}`}</button>
      </footer>
    </div>
  );
}
