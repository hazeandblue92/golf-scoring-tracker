...
export function AdminEventScoring() {
  ...
  const query = useQuery({ queryKey: ['control-room', eventId], refetchInterval: 10_000, queryFn: async () => {
    ...
    const [
      ...
      supabase.from('team_hole_scores').select('event_team_id,event_hole_id,score_status,revision').eq('event_id', eventId),
    ]);
    const roundIds = (rounds ?? []).map((round) => round.id);
    const { data: attestations } = roundIds.length
      ? await supabase.from('scorecard_attestations').select('event_entry_id,event_team_id,score_revision').in('round_id', roundIds)
      : { data: [] };
    ...
    return { event, round: rounds?.[0], entries: entries ?? [], teams: teams ?? [], competitions: competitions ?? [], conflicts: conflicts ?? [], scores: scores ?? [], teamScores: teamScores ?? [], attestations: attestations ?? [], currentProjection };
  }});
  ...
  const { event, round, entries, teams, competitions, conflicts, scores, teamScores, attestations, currentProjection } = query.data;
  const teamBall = competitions.some((competition) =>
    ['scramble', 'foursomes', 'greensomes', 'chapman'].includes(competition.format));
  const cardCount = teamBall ? teams.length : entries.length;
  const activeScores = teamBall ? teamScores : scores;
  const required = (round?.hole_count ?? 0) * cardCount;
  ...
  for (const score of activeScores) {
    ...
    cardRevisions.set(targetId, (cardRevisions.get(targetId) ?? 0) + score.revision);
  }
  const attestedCards = new Set(attestations
    .filter((attestation) => {
      const targetId = teamBall ? attestation.event_team_id : attestation.event_entry_id;
      return targetId && Number(attestation.score_revision) === (cardRevisions.get(targetId) ?? 0);
    })
    .map((attestation) => teamBall ? attestation.event_team_id : attestation.event_entry_id)).size;
  ...
  return <div className="screen control-screen">
    ...
    <section className="progress-panel"><div><h2>Field progress</h2><strong>{received} / {required}</strong></div><progress className="progress-track" aria-label="Field score completion" value={progress} max={100}>{progress}%</progress><p>{progress}% received · {Math.max(0, required - received)} missing · {attestedCards}/{cardCount} {teamBall ? 'team ' : ''}cards attested</p></section>
