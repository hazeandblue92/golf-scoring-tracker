...
| AC-006 | Partial | The production-PWA browser journey caches an event, queues a score offline, reloads from IndexedDB, reconnects, and proves the original idempotency key syncs exactly once. It now passes in all four browser profiles, WebKit and Pixel 7 included; before 2026-08-26 the offline reload stranded the player, because a request that stalls rather than errors left the profile lookup and the snapshot read waiting forever while `navigator.onLine` still reported a connection. Both reads are bounded and fall back to cache, and a stalled read now marks the app offline app-wide. Screen-lock survival and a real Safari/iPhone offline deep-link reload remain device drills: WebKit under Playwright is not a physical handset. |
...
| AC-009 | Demonstrated | Phase 1 workflow coverage proves missing-score blocking/override; `tests/integration/test/conflict.test.ts` proves open conflicts block finalization without an override. `tests/integration/test/best-ball-completeness.test.ts` proves a team competition cannot seal on one member card while a partner's is blank, and that a terminal status still counts as resolved. |
...
| AC-REL-002 | Demonstrated | `tests/integration/test/conflict.test.ts` proves stale writes create durable, explicit conflicts without overwriting the score. The browser retains the conflict id and closes the local row once an organizer resolves it, so a lost conflict stops counting as unsynced work; that reconciliation is covered by unit tests only and has no integration coverage yet. |
| AC-REL-003 | Demonstrated | `tests/integration/test/projection-recovery.test.ts` forces projection publication to defer, proves the raw fact and idempotent receipt survive, then verifies repair converges to the expected leaderboard row. |
| AC-REL-004 | Demonstrated | `tests/e2e/phase1.spec.ts` silences Realtime delivery, observes the ten-second polling fallback converge, and verifies the underlying score mutation exists exactly once. |
