...
| `flights` | Flights/divisions | `event_id` + `name` (unique), `sort_order`, `eligibility_json` | Read with event. Write: directors |
| `event_entries` | Enrollment with frozen handicap: source, `handicap_value numeric(5,1)`, `course_handicap_unrounded numeric(12,6)`, `playing_handicap`, `allowance numeric(5,4)`, profile, tee snapshot ref, hash | `event_id` + `participant_id` (unique), `flight_id`, `status` (entity_status), handicap freeze columns | Read with event (players see their own frozen snapshot here). Write: directors |
